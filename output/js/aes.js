...
Aes.cipher = function(input, w) {    // main Cipher function [§5.1]
    ...
    for (i=4*Nb-1; i>=0; i--) state[i%4][Math.floor(i/4)] = input[i];
    ...
    for (i=4*Nb-1; i>=0; i--) output[i] = state[i%4][Math.floor(i/4)];
    return output;
};
...
Aes.subBytes = function(s, Nb) {    // apply SBox to state S [§5.1.1]
    for (var r=3; r>=0; r--) {
        for (var c=Nb-1; c>=0; c--) s[r][c] = Aes.sBox[s[r][c]];
    }
    return s;
};
 ...
 * @param {String} mynonce   The startup nonce
...
Aes.Ctr.encrypt = function(plaintext, password, nBits, mynonce) {
    ...
    if (mynonce) for (i=0;i<8;i++) counterBlock[i] = mynonce.charCodeAt(i);
...
Utf8.encode = function(strUni) {
    return unescape(encodeURIComponent(strUni));
};
